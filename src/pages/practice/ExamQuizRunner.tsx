...
// Exam delivery mode (PRD §8.3): strict countdown, free navigation with a
// question palette, flagging, no feedback of any kind before submission, one
// submit for the whole set, auto-submit exactly once on expiry. Marking stays
// server-side per item (independent cells, idempotent submission ids).
export function ExamQuizRunner({
  ...
  const totalSeconds = useMemo(() => {
    ...
    if (quiz.est_minutes) return quiz.est_minutes * 60
    return items.length * 120 // FA pacing: ~2 minutes per 2-mark OT
  }, [search, quiz.est_minutes, items.length])

  const examKey = `exam:${userId}:${quiz.id}`
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (saved?.deadline && saved.deadline > Date.now()) {
        ...
        localStorage.setItem(examKey, JSON.stringify({ deadline: dl, flags: [] }))
      }
    } catch {
      /* fresh state */
    }
    setDeadline(dl)
  }, [items, userId, examKey, totalSeconds])
  ...
  const submitAll = useCallback(
    ...
    [items, sessionId, answers, userId, examKey],
  )
              ...
              setFlags((prev) => {
                ...
                try {
                  localStorage.setItem(
                    examKey,
                    JSON.stringify({ deadline, flags: Array.from(next) }),
                  )
                } catch {
                  /* best effort */
                }
